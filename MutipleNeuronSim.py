...
class NeuronB():
    def __init__(self,phiB,muB,T,M, neuronA,poisson_measure):
        ...
        self.history = self.events()
    def events(self):
        eventsA = neuronA.events()
        return new_model.simulate_hawkes_semilinear_finite2D(muB=self.muB, muA=self.neuronA.mu,
                                                             phiB=self.phiB,poisson_measure=self.poisson_measure,
                                                             eventsA = neuronA.history)
    def intensity_values(self):
        time_scale = np.linspace(0,self.T,T*NUMBER_OF_POINTS_PER_UNIT_OF_TIME)
        intensity_values = np.zeros_like(time_scale)
...
if __name__ == "__main__":
    ...
    muA = 1.5
    muB = 4
    phiA = lambda x: basicfunctions.exponential_kernel(x,alpha=2,beta=4)
    phiB = lambda x: basicfunctions.exponential_kernel(x,alpha=3,beta=3.5)
